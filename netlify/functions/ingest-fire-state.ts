/**
 * STATE/NFIRS FIRE INCIDENT INGESTION FUNCTION
 * 
 * Ingests fire incidents from state fire marshal or NFIRS data sources.
...
const SOURCE_NAME = 'fire_state';
const CONFIDENCE_BASE = 0.85; // State/NFIRS data has higher confidence
...
async function fetchFireIncidents(startDate: Date, endDate: Date): Promise<FireIncidentSource[]> {
  const apiUrl = process.env.FIRE_STATE_API_URL!;
  const apiKey = process.env.FIRE_STATE_API_KEY!;
  
  try {
    console.log(`Fetching state/NFIRS fire incidents from ${startDate.toISOString()} to ${endDate.toISOString()}`);
    ...
    console.error('Error fetching state/NFIRS fire incidents:', error);
    throw error;
  }
}
...
function normalizeSeverity(incident: FireIncidentSource): number {
  ...
  // Default moderate severity for state/NFIRS data
  return 0.60;
}
...
function normalizeFireIncident(incident: FireIncidentSource): any {
  ...
  return {
    ...
    // Confidence and priority (higher for state/NFIRS data)
...
async function ingestStateFireIncidents(): Promise<IngestionResult> {
  ...
  try {
    ...
    console.log(`Retrieved ${incidents.length} state/NFIRS fire incidents`);
...
export const handler: Handler = async (event: HandlerEvent) => {
  console.log('🔥 Starting state/NFIRS fire incident ingestion...');
  
  // Fail fast if environment variables missing
  if (!process.env.FIRE_STATE_API_URL || !process.env.FIRE_STATE_API_KEY) {
    const error = 'Missing required environment variables: FIRE_STATE_API_URL or FIRE_STATE_API_KEY';
    console.error('❌', error);
    return {
      statusCode: 500,
      body: JSON.stringify({
  ...
  try {
    const result = await ingestStateFireIncidents();
    ...
    console.log('🎉 State/NFIRS fire incident ingestion complete');
    ...
    console.error('❌ State/NFIRS fire incident ingestion failed:', error);
